...
ChatLogic::~ChatLogic()
{
    ...
    // VISI: since smart pointer, it will be deleted by itself
    // TASK 2
    // delete _chatBot;

    // VISI: Smart pointer will delete itself
    // TASK 3
    /*// delete all nodes
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // VISI: this code is adapted for the unique pointer  -- TASK 3
                        // VISI: since unique pointer can't be copied
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // TASK 3  -- adapt for smart pointer
                            ...
                            // create new edge
                            // TASK 4: make it unique pointer
                            // GraphEdge *edge = new GraphEdge(id);
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);

                            // TASK 3- convert to raw pointer
                            ...
                            // TASK 4: transfer ownership not required , as owned by graph node
                            ...
                            // TASK 4: as edge is now the unique pointer, 
                            // hence send the raw pointers
                            // (*childNode)->AddEdgeToParentNode(edge);
                            // (*parentNode)->AddEdgeToChildNode(edge);

                            (*childNode)->AddEdgeToParentNode(edge.get());
                            // TASK 4: _child node is owned by Graph node
    ...
    // identify root node
  	// TASK 5: Turn it into unique pointer
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
     ...
     // create instance of chatbot
  	// TASK 5: move to local variable
    ChatBot memBot("../images/chatbot.png");

    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
  	// TASK 5: move to local variable
    memBot.SetChatLogicHandle(this);

    // add chatbot to graph root node
  	// TASK 5
    // _chatBot->SetRootNode(rootNode);
  	memBot.SetRootNode(rootNode);
    // TASK 5: move the ownership 
    // rootNode->MoveChatbotHere(std::move(_chatBot));
    rootNode->MoveChatbotHere(std::move(memBot));
